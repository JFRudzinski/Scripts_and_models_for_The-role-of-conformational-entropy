...
Tmle = []
for pair in range(N_pairs):
    lcc = pyemma.msm.estimation.largest_connected_set(Cmat_pairs[pair], directed=True)
    Cmat_pairs[pair] = pyemma.msm.estimation.largest_connected_submatrix(Cmat_pairs[pair], directed=True, lcc=lcc)
    Tmle.append( pyemma.msm.estimation.transition_matrix(Cmat_pairs[pair], reversible=True) )

# Now save all the model
np.save('T2res_seqdep',Tmle)
