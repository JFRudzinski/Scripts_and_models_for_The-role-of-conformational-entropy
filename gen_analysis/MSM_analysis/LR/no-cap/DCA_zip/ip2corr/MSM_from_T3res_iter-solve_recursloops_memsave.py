...
cckp1res = np.load('../../../../DCA/cc3res.npy')
cckres = np.load('../../../../DCA/cc2res.npy')
cc_full = np.load('../../../../DCA/cc_full.npy')
...
for res in range(Nres-(Ncorr-1)):
    ...
    for kp1state in range(len(states_kp1res)):
        ind1 = np.where( np.all( states_kres==states_kp1res[kp1state][:-1], axis=1 ) )[0][0]
...
if ( flag_bndry_cond ):
    ...
    print 'trying to substitute inp_bndry_cond into eqns_Pkres...'
    eqns_Pkres = sub_inp( eqns_Pkres, inp_bndry_cond )
    #save_object('eqns_Pkres_sub_inp_bndry_cond.pkl', eqns_Pkres)
    print 'done with inp_bndry_cond'
    print 'trying to substitute inp_bndry_cond_stat into eqns_Pkres...'
    eqns_Pkres = sub_inp( eqns_Pkres, inp_bndry_cond_stat )
    #save_object('eqns_Pkres_sub_inp_bndry_cond_stat.pkl', eqns_Pkres)
    print 'done with inp_bndry_cond_stat'
    print 'trying to substitute inp_bndry_cond_kres into eqns_Pkres...'
    eqns_Pkres = sub_inp( eqns_Pkres, inp_bndry_cond_kres )
    #save_object('eqns_Pkres_sub_inp_bndry_cond_kres.pkl', eqns_Pkres)
    print 'done with inp_bndry_cond_kres'
    print 'trying to substitute inp_bndry_cond_kres_stat into eqns_Pkres...'
    eqns_Pkres = sub_inp( eqns_Pkres, inp_bndry_cond_kres_stat )
    #save_object('eqns_Pkres_inp_bndry_cond_kres_stat.pkl', eqns_Pkres)
    print 'done with the bndry conditions'

# also replace permanent (i.e., not updated during the iterations) inputs
print 'trying to substitute inp_var into eqns_Pkres...'
eqns_Pkres = sub_inp( eqns_Pkres, inp_var )
save_object('eqns_Pkres_subbed.pkl', eqns_Pkres)
print 'done with all the permanent subs'
