...
Alpha_CG = np.load('Alpha_unf_mpp.npy')
...
for path in paths_CG_unf[0]:
    ...
    for node in range(len(path)-2):
        ...
        for ind,node2 in enumerate( np.delete(np.arange(TNres_CG.shape[0]),[path[node+1]]) ):
            ...
            if ( m_ind > path[node+1] ):
                m_ind -= 1
            H_tmp += M_F[path[node],m_ind]*np.sum(H_row)
    # calc H(T)ld
    l = len(path)-2
    ...
    for ind,node2 in enumerate( np.delete(np.arange(TNres_CG.shape[0]),[d]) ):
        ...
        if ( m_ind > d ):
            m_ind -= 1
        H_tmp += M_F[path[l],m_ind]*np.sum(H_row)
    H_path_cond_CG.append(H_tmp)

np.save('H_path_cond_CG_unf_mpp.npy', H_path_cond_CG)
...
np.save('H_path_cond_state_CG_unf_mpp.npy', H_path_cond_state_CG)
