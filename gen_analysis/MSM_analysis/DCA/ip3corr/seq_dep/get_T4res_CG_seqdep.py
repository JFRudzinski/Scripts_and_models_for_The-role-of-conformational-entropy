...
Tmle = []
for quad in range(N_quads):
    lcc = pyemma.msm.estimation.largest_connected_set(Cmat_quads[quad], directed=True)
    Cmat_quads[quad] = pyemma.msm.estimation.largest_connected_submatrix(Cmat_quads[quad], directed=True, lcc=lcc)
    Tmle.append( pyemma.msm.estimation.transition_matrix(Cmat_quads[quad], reversible=True) )

# Now save all the model
np.save('T4res_seqdep',Tmle)
